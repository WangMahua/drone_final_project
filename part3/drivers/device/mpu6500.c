...
void mpu6500_init(imu_t *imu)
{
	...
	while((mpu6500_read_who_am_i() != 0x71));    // mpu6500 -> 0x70,  mpu9250 -> 0x71
	...
	// while(mpu6500.init_finished == false);   // because no need to calibrate
}
...
void debug_print_mpu6500_gyro(void)
{
	char s[100] = {0};

	sprintf(s, "[gyro] x:%4d, y:%4d, z:%4d\n\r", imu_mpu6500->gyro_unscaled[0],
	        imu_mpu6500->gyro_unscaled[1], imu_mpu6500->gyro_unscaled[2]);
